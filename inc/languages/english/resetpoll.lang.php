<?php
/**
 * Reset Poll
 * Copyright 2010 Starpaul20
 */

$l['resetpoll_info_name'] = "Reset Poll";
$l['resetpoll_info_desc'] = "Allows moderators and administrators to reset a poll.";

$l['reset_poll'] = "Reset poll";
$l['poll_reset'] = "Poll Reset";
$l['reset_poll_confirm'] = "Are you sure you wish to reset this poll? All votes will be deleted.";

$l['error_invalidpoll'] = "Invalid Poll specificed";
$l['error_novotes'] = "This poll has no votes to delete and therefore cannot be reset.";
$l['redirect_pollreset'] = "The poll has been reset and all votes deleted.<br />You will now be returned to the thread.";

?>